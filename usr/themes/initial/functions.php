...
function themeConfig($form) {
	...
	function themeInit($archive) {
		...
		// 注册 IndexNow 推送钩子（必须在 themeInit 内注册，此时 Typecho_Plugin 已加载）
		static $hookRegistered = false;
		if (!$hookRegistered && class_exists('Typecho_Plugin')) {
			Typecho_Plugin::factory('Widget_Contents_Post_Edit')->finishPublish = 'pushIndexNow';
			$hookRegistered = true;
		}
	}
...
function themeFields($layout) {
	...
	$layout->addItem($catalog);
        $sticky = new Typecho_Widget_Helper_Form_Element_Radio('sticky',
        array('1' => _t('置顶'),
        'no' => _t('不置顶')),
        'no', _t('文章置顶'), _t('设为置顶后文章将显示在首页最前面'));
        $layout->addItem($sticky);

	// FAQ 隐形问答字段（右侧 Meta Box，发布前填写，前端隐形输出 FAQPage Schema）
	$faqGroup = new Typecho_Widget_Helper_Form_Element_Textarea('faq_group', NULL, NULL, _t('FAQ 隐形问答（最多 3 组，前端不可见，仅输出 Schema）'), _t('每组占两行：第一行问题，第二行回答。留空则不输出。示例：\n近视手术后会反弹吗？\n正规手术后屈光状态通常稳定，但需注意用眼卫生，避免二次近视。\n\n干眼症能治好吗？\n干眼症可通过药物、物理治疗、生活方式调整缓解，多数可长期控制。\n\n角膜塑形镜适合多大孩子？\n建议 8 岁以上、近视度数 600 度以内、角膜形态正常的儿童青少年。'));
	$faqGroup->input->setAttribute('class', 'w-100 monospace');
	$faqGroup->input->setAttribute('rows', 8);
	$faqGroup->input->setAttribute('placeholder', "近视手术后会反弹吗？\n正规手术后屈光状态通常稳定，但需注意用眼卫生，避免二次近视。\n\n干眼症能治好吗？\n干眼症可通过药物、物理治疗、生活方式调整缓解，多数可长期控制。\n\n角膜塑形镜适合多大孩子？\n建议 8 岁以上、近视度数 600 度以内、角膜形态正常的儿童青少年。");
	$layout->addItem($faqGroup);
}
...
/**
 * 输出完整的 Article Schema + FAQPage Schema（如有）
 * 在 post.php 中调用：postSchema($this)
 */
function postSchema($archive) {
    ...
    // 缩略图
    $image = '';
    if ($thumb = $archive->fields->thumb) {
        if (is_numeric($thumb)) {
            preg_match_all('/<img.*?src="(.*?)".*?[\/]?>/i', $archive->content, $matches);
            if (isset($matches[1][$thumb-1])) {
                $image = $matches[1][$thumb-1];
            }
        } else {
            $image = $thumb;
        }
    }
    if ($options->AttUrlReplace && $image) {
        $list = explode(PHP_EOL, $options->AttUrlReplace);
        foreach ($list as $tmp) {
            list($old, $new) = explode('=', $tmp);
    ...
    $schema = array(
        ...
        'datePublished' => $archive->date('c'),
        'dateModified' => $archive->modified('c'),
        'author' => array(
            '@type' => 'Person',
            'name' => $authorName,
            'url' => $authorUrl,
            ...
            'knowsAbout' => array('眼科', '近视防控', '白内障', '干眼症', '角膜塑形镜', '屈光手术', '视光学'),
        ...
        'mainEntityOfPage' => array(
            '@type' => 'WebPage',
            '@id' => $archive->permalink()
        ),
        'articleSection' => $category,
        'keywords' => implode(',', $tags),
        'description' => $archive->excerpt(150, ''),
        'speakable' => array(
            '@type' => 'SpeakableSpecification',
            'cssSelector' => array('.post-content')
        )
    );
 ...
 */
function pushIndexNow($archive) {
    if (!$archive->is('post') || $archive->status != 'publish') return;

    $keyFile = __TYPECHO_ROOT_DIR__ . '/../indexnow-key.txt';
